...
# Stale run threshold: runs RUNNING with no heartbeat for 10 minutes
STALE_RUN_THRESHOLD_SECONDS = 600
# Max retries for stale runs before marking FAILED
STALE_RUN_MAX_RETRIES = 2
        ...
        for run in stale_runs:
            if run.retry_count < STALE_RUN_MAX_RETRIES:
                ...
                logger.warning(
                    ...
                    run.id, run.retry_count, STALE_RUN_MAX_RETRIES,
                )
            else:
            ...
            session.add(
                OutboxEvent(
                    ...
                    payload={
                        ...
                        "age_days": (now - (req.created_at.replace(tzinfo=timezone.utc) if req.created_at.tzinfo is None else req.created_at)).days,
                    },
                )
            )
            cancelled += 1
            logger.info("Auto-cancelled stale request %s (CREATED for >%d days)", req.id, STALE_CREATED_THRESHOLD_DAYS)
...
async def main():
    logger.info("Reconciler starting — outbox every 5s, stale runs every 30s, ledger every 60s, auto-cancel every 10min")
    outbox_counter = 0
    while True:
        try:
            await run_once()
